...
export abstract class BaseModel extends AbstractModel<ErrorModel> {
    public readonly languageService = container.resolve(ServerLanguageService);
        ...
        if (me.errors.length > 0) {
            for (const error of me.errors) {
                if (error.property === prop) {
                    errorMessage += '<span class="label-text text-danger">' + error.error + '</span>';
                }
            }
        }
        return '<label class="label flex-col items-start">' + errorMessage + '</label>';
    }
}
